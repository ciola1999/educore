...
export function AddSemesterDialog({ onSuccess }: { onSuccess: () => void }) {
  ...
  async function handleSubmit(values: SemesterFormValues) {
    try {
      await apiPost<{ id: string }>("/api/semesters", values);
      ...
      toast.error(
        error instanceof Error ? error.message : "Gagal membuat semester",
      );
    }
  }
      ...
      <DialogContent className="bg-zinc-900 border-zinc-800 text-white">
        <DialogHeader>
          ...
          <DialogDescription className="text-zinc-400">
            Hubungkan semester ke tahun ajaran yang valid.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
