...
export function AcademicYearList({ readOnly = false }: { readOnly?: boolean }) {
  ...
  const [deleteTarget, setDeleteTarget] = useState<AcademicYearItem | null>(
    null,
  );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      setData(await apiGet<AcademicYearItem[]>("/api/academic-years"));
    } catch (error) {
      const message =
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await apiDelete(`/api/academic-years/${deleteTarget.id}`);
  ...
  if (errorMessage && data.length === 0) {
    return (
      <InlineState
        title="Data tahun ajaran belum tersedia"
    ...
    <div className="space-y-4">
      {readOnly ? (
        ...
        <div className="flex justify-end">
          <AddAcademicYearDialog onSuccess={fetchData} />
        </div>
      )}
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-zinc-800 hover:bg-zinc-900">
                ...
                <TableHead className="text-zinc-400">Mulai</TableHead>
                <TableHead className="text-zinc-400">Selesai</TableHead>
              ...
              {data.length === 0 ? (
                ...
                data.map((item) => (
                  ...
                  >
                    ...
                    <TableCell>{item.tanggalMulai.slice(0, 10)}</TableCell>
                    <TableCell>{item.tanggalSelesai.slice(0, 10)}</TableCell>
                    <TableCell>
          ...
          <AlertDialogContent className="bg-zinc-900 border-zinc-800 text-white">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="text-zinc-400">
                {deleteTarget
                  ? `Tahun ajaran ${deleteTarget.nama} akan dihapus dari master data akademik.`
                  : "Tahun ajaran akan dihapus."}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel
